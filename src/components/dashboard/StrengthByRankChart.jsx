...
const RANK_COLORS = [
  ...
  "#4A6B3A", "#8A6B4A", "#2D5A3A", "#6B5A3A", "#3A7A4A", "#7A6A4A", "#5A4A3A",
];

export default function StrengthByRankChart({ data = [] }) {
  if (!data.length) {
    return (
      ...
      <ResponsiveContainer width="100%" height={Math.max(200, data.length * 40)}>
        <BarChart data={data} margin={{ left: 16, right: 16, top: 8, bottom: 8 }}>
          ...
          <Bar dataKey="count" radius={[4, 4, 0, 0]}>
            {data.map((entry, i) => (
              <Cell key={entry.rank} fill={RANK_COLORS[i % RANK_COLORS.length]} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
